import { ArticyObjectProps, Id } from "articy-node/json";
...
export class FlowBranch
{
     ...
     */
    destinationAs<ObjectType>(type: ArticyObjectCreator<ObjectType>, typeString?: string) : ObjectType|undefined {
     ...
     * @param typeString Type string
     */
    pathHas<ObjectType>(type: ArticyObjectCreator<ObjectType>, typeString: string): ObjectType|undefined {
        // Go through path
 ...
 */
export const NullAdvancedFlowState: AdvancedFlowState = { id: null, last: null, variables: {}, visits: { counts: {}, indicies: {} }, branches: [], turn: 0 };
...
export interface AdvancedIterationConfig 
{
     ...
     */
    customStopHandler?: (node: BaseFlowNode, visits: VisitSet) => CustomStopType|void;
}
 ...
 */
export function getFlowStateChildren(db: Database, state: FlowState, node?: BaseFlowNode): BaseFlowNode[]
{
    if(!state.id) {
        return [];
    }
    ...
    const numChildren = node.numBranches(state.variables, state.last, state.shadowing ?? false);
    for(let i = 0; i < numChildren; i++) {
        const child = node.next(state.variables, i, state.last, state.shadowing ?? false);
        if(child) {
            children.push(child)
        }
    }
 ...
 */
export function basicNextFlowState(db: Database, state: FlowState, branchIndex: number): [FlowState, BaseFlowNode|undefined]
{
    ...
    // Find the next node
    const next = node?.next(state.variables, branchIndex, state.last, state.shadowing ?? false);

    // Nowhere to go.
    if(!next) {
        return [{id: null, last: null, variables: {}, shadowing: state.shadowing}, undefined];
    }

    // Create new state
    return [{ id: next.properties.Id, last: state.id, variables: state.variables, shadowing: state.shadowing }, next]
}

function shouldStopAt(node: ArticyObject<ArticyObjectProps>, stopAtTypes: string[]) {
 ...
 */
export function advancedStartupFlowState(db: Database, start: Id, config: AdvancedIterationConfig): [AdvancedFlowState, BaseFlowNode|undefined]
{
    // Create initial state
    let initial: AdvancedFlowState = { id: start, last: null, branches: [], variables: db.newVariableStore(), visits: { counts: {}, indicies: {} }, turn: 0 };
 ...
 */
export function advancedNextFlowState(db: Database, state: AdvancedFlowState, config: AdvancedIterationConfig, branchIndex: number): [AdvancedFlowState, BaseFlowNode|undefined]
{
...
export function collectBranches(db: Database, iter: FlowState, config: AdvancedIterationConfig, visits: VisitSet, branch?: FlowBranch, index = 0, direction = -1, node?: BaseFlowNode): FlowBranch[] {
    ...
    let branches = node.numBranches(iter.variables, iter.last, iter.shadowing ?? false);
    ...
    while(branches === 1 || direction >= 0) {
        ...
        const limit_feature = node.template?.Visit_Limitation as Visit_Limitation;
        ...
        if(shouldStopAt(node, config.stopAtTypes)) {
            ...
            if(config.customStopHandler) {
                ...
                if(behaviour === CustomStopType.NormalStop || behaviour === undefined) {
                    return [branch];
                }
                    ...
                    return [branch, ...collectBranches(db, iter, config, visits, forked, index + 1, 0, node)];
                }
                else if(behaviour === CustomStopType.Continue) {
                    // Continue. Do nothing
                } else {
                    // Unexpected type... stop?
                    console.log(`Unexpected custom stop behaviour found: ${behaviour}. Unsure what to do.... Stopping here.`);
                    return [branch];
                }
        ...
        // Continue
        branches = node.numBranches(iter.variables, iter.last, iter.shadowing ?? false);
    }
    ...
    for(let i = 0; i < branches; i++) {
        ...
        // Go!
        result = [...result, ...collectBranches(db, iter, config, visits, forked, index, i, node)];
 ...
 */
export function refreshBranches(db: Database, state: AdvancedFlowState, config: AdvancedIterationConfig): AdvancedFlowState {
    const result = {...state};
    const vars = state.variables;
    result.branches = collectBranches(db, {...state, shadowing: true}, config, state.visits);
