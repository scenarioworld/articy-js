import { ArticyData, ArticyObjectProps, EnumDefinition, FlowFragmentProps, HierarchyEntry, Id, ModelData, TemplateProps } from "articy-node/json";
...
export class Database
{
    ...
    constructor(data: ArticyData, assetResolver?: ResolveAssetPath) {
        ...
        for(const pkg of this._data.Packages) {
            ...
            for(const model of pkg.Models) {
                ...
                if("InputPins" in model.Properties || "OutputPins" in model.Properties)
                {
     ...
     */
    public getEnumValueDisplayName(type: string, value: number): string|undefined {
        // Lookup enum
    ...
    public getChildrenOfType<ObjectType>(objectId: Id, creator: ArticyObjectCreator<ObjectType>): ObjectType[]
    {
        return this.getChildren(objectId).map(id => this.getObject<ObjectType>(id, creator)).filter(obj => obj !== undefined) as ObjectType[];
    }
    ...
    public isOfType(id: Id, creator: ArticyObjectCreator): boolean
    {
        ...
        if(this.isType(def.Type, Database.InverseRegisteredTypes.get(creator) ?? "")) {
            return true;
        }
    ...
    public newVariableStore(): VariableStore {
        ...
        for(const ns of this._data.GlobalVariables)
        {
            ...
            for(const v of ns.Variables)
            {
                ...
                if(value === undefined) {
                    console.log(`Failed to parse variable ${ns.Namespace}.${v.Variable}'s type: ${v.Type}`);
                    continue;
                }
    ...
    public getProperties<PropsType extends ArticyObjectProps>(id: Id): PropsType|undefined {
        const def = this._lookup.get(id);
        if(!def) {
            return undefined;
        }
     ...
     * @param type Type name
     */
    public getPropertiesOfType<PropsType extends ArticyObjectProps>(type: string): PropsType[] {
     ...
     */
    public getObject<ObjectType>(id: Id|null|undefined, type: ArticyObjectCreator<ObjectType>): ObjectType|undefined {
        if(!id) {
            return undefined;
        }
     ...
     */
    public getModelsOfType<PropType extends ArticyObjectProps, TemplateType extends TemplateProps = TemplateProps>(type: string|string[]) : ModelData<PropType, TemplateType>[] {
    ...
    public static readonly RegisteredTypes: Map<string, ArticyObjectCreator> = new Map();
    public static readonly InverseRegisteredTypes: Map<ArticyObjectCreator, string> = new Map();
}
 ...
 */
export function RegisterDatabaseTypeClass<ObjectType>(name: string, creator: ArticyObjectCreator<ObjectType>): void {
