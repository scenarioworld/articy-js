...
import { FeatureProps, ScriptMethodDef } from "articy-node/json";
...
type ApplicationState = ExtensionTypes extends { ApplicationState: unknown } ? ExtensionTypes["ApplicationState"] : unknown;
...
function wrapScriptFunction(func: ScriptFunction, state: Readonly<ApplicationState>|undefined, db: Database, shadowing: boolean): ScriptFunctionInternal {
    return (...args) => {
...
export function VerifyRegisteredScriptMethod(method: ScriptMethodDef): boolean
{
    if(!(method.Name in registeredFunctions)) {
        console.error(`Script method ${method.Name} is used in this Articy Database but is not registered in code!`);
        return false;
    }
...
type FeatureExecutionHandler<Feature extends FeatureProps = FeatureProps> 
    = (db: Database, feature: Feature, node: BaseFlowNode, state: AdvancedFlowState) => Variable|void|ScriptGenerator;
 ...
 */
export function RegisterFeatureExecutionHandler<Feature extends FeatureProps>(name: string, handler: FeatureExecutionHandler<Feature>): void {
 ...
 */
export function OnNodeExecution(node: BaseFlowNode, state: AdvancedFlowState): void {
    if(!node.template) {
        return;
    }
 ...
 */
export function runScript(script: string|undefined, variables: VariableStore, db: Database, returns: boolean, shadowing: boolean): boolean {
    ...
    const func = new Function("window", ...names, ...funcNames, `"use strict"; ${returns ? 'return' : ''} ${script}`);

    // Call the function with the variable sets
    const result = func.call(undefined, undefined, ...names.map(n => variables[n]), ...funcNames.map(f => wrapScriptFunction(registeredFunctions[f], state, db, shadowing)));
