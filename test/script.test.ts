...
import { ClearRegisteredFeatureHandlers, ClearRegisteredScriptFunctions, RegisterScriptFunction, runScript, scriptDispatchMiddleware, ScriptFunction } from '../src/script';

// Clear registrations
beforeEach(() => { ClearRegisteredFeatureHandlers(); ClearRegisteredScriptFunctions(); })
...
describe("A simple function with no return value", () => {
    ...
    beforeEach(() => { RegisterScriptFunction("func", func)});
...
describe("A simple function that returns true", () => {
    // Register simple script function
    const func = jest.fn<ReturnType<ScriptFunction>, Parameters<ScriptFunction>>(() => true)
    beforeEach(() => { RegisterScriptFunction("func", func)});
...
describe("A script environment with a single variable MyNamespace.Variable", () => {
    ...
    test("Script can access variable", () => {
        const result = runScript("MyNamespace.Variable < 44", vars, db, true, false);
        expect(result).toBe(false);
    });
});
...
describe("Script middleware", () => {
    ...
    const executeScript = <T extends AnyAction>(action: T) => { runScript("func()", {}, db, false, false); return action; }
